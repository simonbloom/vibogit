    ...
    return renderWithModal(
      <>
        <div className="flex items-center gap-1.5">
          <button
            type="button"
            onClick={() => diagnosis && setDiagnosticModalOpen(true)}
            className="flex items-center gap-1.5 px-2 py-1 text-sm text-destructive hover:bg-destructive/10 rounded-md transition-colors cursor-pointer"
          >
            <AlertTriangle className="w-3.5 h-3.5" />
            <span className="truncate max-w-[250px]">{diagnosis?.problem || "Failed"}</span>
          </button>
        ...
        {diagnosis && (
          <DiagnosticModal
            ...
            onRunCommand={sendCommandToTerminal}
            diagnosis={diagnosis}
