    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="relative">
          ...
          />
          <Folder className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {quickPaths.map((quick) => (
            <Button
              key={quick.id}
              variant={selectedQuick === quick.id ? "default" : "secondary"}
              size="sm"
        ...
        <div className="bg-muted rounded-lg border border-border divide-y divide-border">
          {detectedApps.map((app) => (
            ...
            >
              ...
              >
                {selectedApp === app.id && <Check className="w-2.5 h-2.5 text-primary-foreground" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={clsx(
                    "text-sm font-medium",
                    selectedApp === app.id ? "text-primary" : "text-foreground"
                  )}>
                    {app.name}
                  </span>
                </div>
